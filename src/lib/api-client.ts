...
apiClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // Jika bukan error 401 atau request sudah pernah dicoba ulang (_retry)
    if (!error.response || error.response.status !== 401 || originalRequest._retry) {
      return Promise.reject(error)
    }

    // Jangan refresh jika request itu sendiri adalah endpoint login atau refresh
    ...
    if (isRefreshing) {
      // Jika refresh token sedang berlangsung, masukkan request ke antrean
    ...
    try {
      // Gunakan axios instance plain tanpa interceptor loop untuk refresh
